...
async function myScannerName({ request, response }) {
    ...
    let memory = null;
    // request.scanContext is injected by run.js
    const { scanContext } = request || {};

    if (scanContext && scanContext.scanId && scanContext.applicationId) {
        memory = new ScanMemory({
            scannerName: 'my-scanner-name', // TODO: Change this to unique scanner name
    ...
    // Identify a unique key for the resource (e.g. req.url)
    const uniqueKey = req.url;

    if (memory && memory.hasSeen(uniqueKey)) {
        // [~] Skipping duplicate
        return null;
    }

    if (memory) {
        memory.store(uniqueKey);
    }
    // ----------------------------
